...
SMAC::SMAC() : Mac(), mhNav_(this), mhNeighNav_(this), mhSend_(this), mhRecv_(this), mhGene_(this), mhCS_(this), syncFlag_(0) {
  ...
  collision_before = 0;
	...
	*/
	...
	printf("timeWaitCtrl_: %0.6lf \n",timeWaitCtrl_);
	printf("max cwtime : %0.6f \n",CLKTICK2SEC(difs_) + DATA_CW*slotTime_sec_);
  // timeWaitCtrl_ =0;
	numSched_ = 0;
	numNeighb_ = 0;
	...
	if (mhCounter_[id]->value_ == sleepTime_) { //woken up from sleep
		...
		if (!(mhNav_.busy()) && !(mhNeighNav_.busy()) &&
			...
			if (schedTab_[id].txSync == 1) {
				...
				// int cwd = SYNC_CW;
				// int i ;
				// for(i =0 ; i < collision_before;i++){
				// 		cwd = cwd/2;
				// }
				// printf("cwd: %d \n",cwd);
				double cw = (Random::random() % SYNC_CW) * slotTime_sec_;
				if(NOW >= 150 && NOW <=850)
				printf("%d time cr: %.6lf \n",index_,CLKTICK2SEC(difs_) + cw);
				mhCS_.sched(CLKTICK2SEC(difs_) + cw);
			}
		}
		// start to listen now
...
void SMAC::transmit(Packet *p) {
  ...
  switch(sh->type) {
	...
	case SYNC_PKT:
		if(NOW >= 150 && NOW <=850)
    printf("sync %d %.6lf ",index_,CLKTICK2SEC(SYNCPKTTIME));
		break;

  case DATA_PKT:
		if(NOW >= 150 && NOW <=850)
    printf("data %d %.6lf ",index_,durDataPkt_);
		break;
  case RTS_PKT:
		printf("rts %d %.6lf ",index_,durCtrlPkt_);
		break;
  case CTS_PKT:
		printf("cts %d %.6lf ",index_,durCtrlPkt_);
		break;
  case ACK_PKT:
    printf("ack %d %.6lf ",index_,durCtrlPkt_);
		break;
...
int SMAC::startUcast()
{
printf("node: %d ..............data sent Uni............ %.6lf\n",index_,Scheduler::instance().clock());
...
void SMAC::sleep() 
{
	...
	if(Scheduler::instance().clock() >= 150  && Scheduler::instance().clock() <= 850){
		// if(state_ != SLEEP){
			printf("active %d %0.6f\n",index_,Scheduler::instance().clock() - time_of_state);
			
		// }
	}
	time_of_state = Scheduler::instance().clock();
...
void SMAC::wakeup()
{
  ...
  //wakeup from sleep. turn on radio
		if(Scheduler::instance().clock() >= 150 && Scheduler::instance().clock() <= 850){
			//if(state_ == SLEEP){
				printf("sleep %d %.6f\n",index_,Scheduler::instance().clock() - time_of_state);
			//	printf("time sleep: %ld\n",Scheduler::instance().clock() - time_of_state);
...
double SMAC::txtime(Packet *p)
{
  ...
  case SYNC_PKT:
    return CLKTICK2SEC(SYNCPKTTIME);
			//return CLKTICK2SEC(durSyncPkt_);
  default:
