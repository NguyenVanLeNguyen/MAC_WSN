...
if ($x[0] eq 's'  && $x[24] =~ /^$tonode/ && $x[3] eq 'AGT' && $x[23] =~ /^\[$sendnode/ ) {
	$pkts_= substr $x[27], 1, -1;	
	
	if(!$send[$pkts_]){
		$send[$pkts_] = $x[1];
		$max_pktid = $pkts_ if ($max_pktid < $pkts_);
	}
	# print STDOUT "send: $x[1] $x[27] \n";
}
if ($x[0] eq 'r'  && $x[24] =~ /^$tonode/ && $x[3] eq 'AGT' && $x[23] =~ /^\[$sendnode/ ) {
	$pktr_= substr $x[27], 1, -1;
	$recv[$pktr_] = $x[1];
	$num++;
	# print STDOUT "recv: $x[1] $x[27]  \n";
}
}
$result;
$delay = 0;
